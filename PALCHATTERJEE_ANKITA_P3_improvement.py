# -*- coding: utf-8 -*-
"""
Created on Wed Oct 16 22:46:01 2019
...
#random.seed(4)
#random.shuffle(datafile.values)
#print(datafile)
#Shuffle the file
...
def hypothesis(w0,w1,w2,w3,w4,w5,x1,x2):
	...
	for i in range(len(x1)):
		hx[i] = w0+(w4*x1[i])+(w5*x2[i])+(w3*x1[i]*x2[i])+(w1*(x1[i])**2)+(w2*(x2[i]**3))
		lx[i] = 1/(1+(np.exp((-1)*(hx[i]))))
	return lx

###############################################################
def cost(w0,w1,w2,w3,w4,w5,x1,x2,pred_train,lx):
	tot_count = len(pred_train)
	Jx = 0.0
	#lx = hypothesis(w0,w1,w2,w3,w4,w5,x1,x2)
	small_val = (1e-5)
	for i in range(len(pred_train)):
		#Jx +=  pred_train[i]*np.log(lx[i]+small_val) + (1-pred_train[i])*np.log(1-lx[i]+small_val)
...
#	temp5 = 0.0
	...
	Jx = cost(w0,w1,w2,w3,w4,w5,inp_data_train_x1,inp_data_train_x2,pred_train,hx)
	...
	for j in range(1,iterations):
		...
		for i in range(len(inp_data_train_x1)):
			...
			w2 = w2 - np.sum(alpha*((hx[i]-pred_train[i])*(inp_data_train_x2[i]**3)))
			w3 = w3 - np.sum(alpha*((hx[i]-pred_train[i])*(inp_data_train_x2[i]*inp_data_train_x1[i])))
			w4 = w4 - np.sum(alpha*((hx[i]-pred_train[i])*(inp_data_train_x1[i])))
			w5 = w5 - np.sum(alpha*((hx[i]-pred_train[i])*(inp_data_train_x2[i])))
#		w0 = w0 - temp0
#		w1 = w1 - temp1
#		w2 = w2 - temp2
#		w3 = w3 - temp3
...
def predict_type(w0_n,w1_n,w2_n,w3_n,w4_n,w5_n,inp_data_train_x1,inp_data_train_x2):
	#print('w0_n,w1_n,w2_n,w4_n,w5_n',w0_n,w1_n,w2_n,w3_n,w4_n,w5_n)
	class_type = np.empty(len(inp_data_train_x1))
	hx_prmt = hypothesis(w0_n,w1_n,w2_n,w3_n,w4_n,w5_n,inp_data_train_x1,inp_data_train_x2)
	print(hx_prmt)
...
######################################################################
def confusion_matrix(pred_train,inp_data_train_x1,inp_data_train_x2,w0_n,w1_n,w2_n,w3_n,w4_n,w5_n):
	class_type = predict_type(w0_n,w1_n,w2_n,w3_n,w4_n,w5_n,inp_data_train_x1,inp_data_train_x2)
	print('class_type',class_type,'pred_train',pred_train)
	nonmatch_count = 0
	TP = 0
	TN = 0
	FP = 0
...
def count_error_parameters(pred_train,inp_data_train_x1,inp_data_train_x2,w0_n,w1_n,w2_n,w3_n,w4_n,w5_n):
	error_count = 0
	accuracy = 0
	precision = 0#.000001
	accuracy_p = 0
	...
	TP,TN,FP,FN = confusion_matrix(pred_train,inp_data_train_x1,inp_data_train_x2,w0_n,w1_n,w2_n,w3_n,w4_n,w5_n)
...
######################################################################
def prompt(w0,w1,w2,w3_n,w4_n,w5_n,mean_x1,mean_x2,stdev_x1,stdev_x2):

#	inp_data_test_x1 = list()
#	inp_data_test_x2 = list()
	while True:
		testval_x = float(input('Enter length of body:'))
		testval_y = float(input('Enter length of dorsal fin:'))
...
#
			...
			pred_prmt = predict_type(w0_n,w1_n,w2_n,w3_n,w4_n,w5_n,inp_data_test_x1,inp_data_test_x2)
...
hx_test = hypothesis(w0_n,w1_n,w2_n,w3_n,w4_n,w5_n,inp_data_test_x1,inp_data_test_x2)
print('hx_test',hx_test)
cost_test = cost(w0_n,w1_n,w2_n,w3_n,w4_n,w5_n,inp_data_test_x1,inp_data_test_x2,pred_test,hx_test)
print('\ncost of test data set',cost_test)
plot_fig(inp_data_test_x1,inp_data_test_x2,'Test Data')
count_error_parameters(pred_test,inp_data_test_x1,inp_data_test_x2,w0_n,w1_n,w2_n,w3_n,w4_n,w5_n)
print('\nw0',w0_n,'\nw1',w1_n,'\nw2',w2_n,'\nw3',w3_n,'\nw4',w4_n,'\nw5',w5_n)
prompt(w0,w1,w2,w3_n,w4_n,w5_n,mean_train_x1,mean_train_x2,std_train_x1,std_train_x2)